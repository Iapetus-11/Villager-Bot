...
class Minecraft(commands.Cog):
    ...
    async def mcping(self, ctx, host=None, port: int = None, note: str = None):
        ...
        async with ctx.typing():
            async with self.ses.get(f'https://betterapi.net:80/mc/mcping?host={combined}&k={self.d.k}') as res:  # fetch status from api
                jj = await res.json()
        ...
        embed.set_image(url=f'https://betterapi.net:80/mc/mcpingimg?host={combined}&imgonly=true&v={random.random()*100000}&k={self.d.k}')

        if jj['favicon'] is not None:
            embed.set_thumbnail(url=f'https://betterapi.net:80/mc/serverfavi?host={combined}&k={self.d.k}')
