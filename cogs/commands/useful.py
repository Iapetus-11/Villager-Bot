...
class Useful(commands.Cog):
    ...
    async def information(self, ctx):
        ...
        info_msg.set_author(name="Villager Bot Information",
                            icon_url="http://172.10.17.177/images/villagerbotsplash1.png")
        await ctx.send(embed=info_msg)

    @commands.command(name="ping", aliases=["pong", "ding", "dong", "shing", "shling",
                                            "schlong"])  # Checks latency between Discord API and the bot
    async def ping(self, ctx):
        ...
        if "pong" in c:
            ...
            await ctx.send(embed=discord.Embed(color=discord.Color.green(),
                                               description=f"<a:ping:692401875001278494> Magnum Dong! \uFEFF ``{69.00} ms``"))
            return
        await ctx.send(embed=discord.Embed(color=discord.Color.green(),
    ...
    async def vote_link(self, ctx):
        _vote_link = discord.Embed(description="[**Click Here!**](https://top.gg/bot/639498607632056321/vote)\n\n"
                                               "[**And Here Too!**](https://discordbotlist.com/bots/villager-bot/upvote)",
    ...
    async def google_search(self, ctx, *, query: str):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(color=discord.Color.green(), description="No results found for that query!"))
            return
        except async_cse.search.APIError:
            await ctx.send(embed=discord.Embed(color=discord.Color.green(),
                                               description="Uh Oh, there was an error, please try again later!"))
            return
        if len(rez) == 0:
            await ctx.send(
                embed=discord.Embed(color=discord.Color.green(), description="No results found for that query!"))
    ...
    async def youtube_search(self, ctx, *, query: str):
        ...
        if rez is None:
            ...
            embed = discord.Embed(color=discord.Color.green(), title=rez.title, description=rez.description,
                                  url=rez.url)
    ...
    async def image_search(self, ctx, *, query: str):
        ...
        try:
            ...
            await ctx.send(embed=discord.Embed(color=discord.Color.green(),
                                               description=f"[Result for \"{query}\"]({results[0].image_url})"))
    ...
    async def nsfw_image_search(self, ctx, *, query: str):
        if not isinstance(ctx.channel, discord.DMChannel):
            if not ctx.channel.is_nsfw():
                await ctx.send(embed=discord.Embed(color=discord.Color.green(),
                                                   description="Hey, there are kids here! You can only use this command in nsfw channels!"))
                return
        await ctx.trigger_typing()
        try:
    ...
    async def info_2(self, ctx):
        ...
        seconds = diff.seconds + diff.days * 3600 * 24
