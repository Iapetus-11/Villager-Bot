...
use crate::{
    ...
    logic::{items::get_user_items, user_tools::get_user_tools, users::{get_or_create_user, get_user_net_wealth}},
};
...
pub async fn get_profile_command_data(
    ...
    let relevant_items = get_user_items(&mut *db, &user.id, Some(&[
        "Mooderald".into(),
    ])).await?;
    ...
    let mooderald_count = relevant_items.iter().find(|i| i.name == "Mooderald").map(|i| i.amount).unwrap_or(0);
...
mod tests {
    ...
    async fn test_can_vote_and_next_vote_time(mut db: PgPoolConn) {
        ...
        assert!(profile_cmd_data.can_vote);
        assert_eq!(profile_cmd_data.next_vote_time, None);
    }
}
